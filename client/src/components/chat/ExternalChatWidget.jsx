...
export function ExternalChatWidget() {
  useEffect(() => {
    ...
    // Crear el botón de entrada
    const entryBtn = document.createElement('div');
    entryBtn.className = 'live-chat-entry close';
    entryBtn.style.position = 'fixed'; // Cambiado a fixed para mejor posicionamiento
    ...
    entryBtn.onclick = function() {
      ...
      if (this.classList.contains('close')) {
        ...
        if (liveChatIframe) {
          ...
          iframeBox.style.position = 'fixed'; // Cambiado a fixed para mejor posicionamiento
    ...
    return () => {
      const existingBtn = document.querySelector('.live-chat-entry');
      if (existingBtn) {
        existingBtn.remove();
      }
      const existingIframe = document.getElementById('liveChatIframe');
      if (existingIframe) {
